...
export function CourseRow({ course, categoryName, categoryColor = '#7C3AED', onEdit, onDelete, onDuplicate }) {
  const stats = countCourseStats(course);
  return (
    <tr className="border-b border-brand-border dark:border-slate-800 hover:bg-brand-surface/50 dark:hover:bg-slate-850/40">
      <td className="px-4 py-3">
        <Link to={`/catalog/courses/${course.id}`} className="flex items-center gap-3">
          <img src={course.thumbnail} alt="" className="h-10 w-14 rounded-lg object-cover bg-brand-surface dark:bg-slate-950 border border-brand-border dark:border-slate-800" />
          <div>
            <p className="font-semibold text-brand-text-primary dark:text-slate-200 hover:text-accent-teal-dark dark:hover:text-accent-teal">{course.title}</p>
      ...
      <td className="px-4 py-3">
        <div className="flex gap-1">
          <button type="button" onClick={() => onEdit(course)} className="rounded-full p-1.5 hover:bg-brand-surface dark:hover:bg-slate-800 text-brand-text-secondary dark:text-slate-400"><Pencil className="h-4 w-4" /></button>
          <button type="button" onClick={() => onDuplicate(course)} className="rounded-full p-1.5 hover:bg-brand-surface dark:hover:bg-slate-800 text-brand-text-secondary dark:text-slate-400"><Copy className="h-4 w-4" /></button>
          <button type="button" onClick={() => onDelete(course)} className="rounded-full p-1.5 hover:bg-red-50 dark:hover:bg-red-950/20 text-red-600 dark:text-red-400"><Trash2 className="h-4 w-4" /></button>
        </div>
      </td>
    </tr>
  );
