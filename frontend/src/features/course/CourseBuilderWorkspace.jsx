...
import RichNotesEditor from '@/components/ui/RichNotesEditor';
...
export default function CourseBuilderWorkspace({ course, category, students, catalog, showToast }) {
  ...
  const handleAddContent = (moduleId, submoduleId, item = null) => {
    ...
    if (item) {
      setContentForm({
        ...
        linkUrl: item.type === 'link' ? (item.fileUrl || '') : '',
  ...
  const handleSaveContent = () => {
    ...
    if (contentForm.type !== 'notes' && contentForm.type !== 'link' && !contentForm.fileUrl) {
      showToast('Please upload a file first', 'error');
      return;
    }
    if (contentForm.type === 'notes' && !contentForm.markdown.trim()) {
      showToast('Notes body is empty', 'error');
      return;
    }
    ...
    // Resolve Thumbnail (Auto assign if empty)
    let finalThumbnail = contentForm.thumbnail;
    if (!finalThumbnail) {
      finalThumbnail = getAIPlaceholderImage(contentForm.title, contentForm.type);
    }

    const payload = {
      ...contentForm,
      thumbnail: finalThumbnail,
      fileUrl: contentForm.type === 'link' ? contentForm.linkUrl : contentForm.fileUrl
    };

    if (contentForm.id) {
      catalog.updateContent(course.id, contentForm.moduleId, contentForm.submoduleId, contentForm.id, payload);
      showToast('Content updated successfully');
    } else {
      catalog.addContent(course.id, contentForm.moduleId, contentForm.submoduleId, payload);
  ...
  const handleModalFileUpload = async (file) => {
    ...
    if (contentForm.type === 'pdf' && extension !== '.pdf') {
      setContentFileError('Invalid file type. Please upload a PDF document.');
      return;
    }
    if ((contentForm.type === 'ppt' || contentForm.type === 'pptx') && !['.ppt', '.pptx'].includes(extension)) {
      setContentFileError('Invalid file type. Please upload a PowerPoint file.');
      return;
    }
    if ((contentForm.type === 'doc' || contentForm.type === 'docx') && !['.doc', '.docx'].includes(extension)) {
      setContentFileError('Invalid file type. Please upload a Word document.');
      return;
    }
    if (contentForm.type === 'video' && !mime.startsWith('video/')) {
      setContentFileError('Invalid file type. Please upload a video file (MP4, WebM etc).');
      return;
    }
    if (contentForm.type === 'audio' && !mime.startsWith('audio/')) {
      setContentFileError('Invalid file type. Please upload an audio file (MP3, WAV etc).');
      return;
    }
    if (contentForm.type === 'zip' && !['.zip', '.rar', '.7z'].includes(extension)) {
      setContentFileError('Invalid file type. Please upload a ZIP/archive file.');
      return;
    }
    if (contentForm.type === 'image' && !mime.startsWith('image/')) {
      setContentFileError('Invalid file type. Please upload an image file.');
      return;
    }

    if (file.size > 50 * 1024 * 1024) {
      setContentFileError('File size exceeds the 50MB limit.');
      return;
    }
  ...
  const handleModalCancelUpload = () => {
    setContentSelectedFile(null);
    setContentForm(prev => ({ ...prev, fileUrl: '', fileSize: 0 }));
    setContentUploadProgress(0);
    setContentFileError('');
  };
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] bg-brand-surface dark:bg-slate-950 transition-colors">
      ...
      <div className="sticky top-0 z-20 border-b border-brand-border dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md">
        <div className="flex items-center gap-4 px-4 py-3">
          <Link to={`/catalog/categories/${course.categoryId}`} className="p-2 hover:bg-brand-surface dark:hover:bg-slate-800 rounded-lg text-brand-text-secondary dark:text-slate-400">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden xl:flex w-72 shrink-0 flex-col border-l border-brand-border dark:border-slate-800 bg-white dark:bg-slate-900 p-4 overflow-y-auto scrollbar-thin">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-brand-text-primary dark:text-slate-100">Metadata</h3>
            <Link to={`/catalog/courses/${course.id}/edit`} className="text-xs font-semibold text-accent-teal-dark hover:underline">Edit Details</Link>
          </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleSaveContent} disabled={contentUploading}>Save</Button>
          </>
        }
      >
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 scrollbar-thin">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Select 
              ...
              onChange={(e) => {
                setContentForm({ ...contentForm, type: e.target.value, fileUrl: '', fileSize: 0, markdown: '' });
                setContentSelectedFile(null);
                setContentFileError('');
              }} 
          ...
          <TextArea 
            ...
            placeholder="Brief explanation of this block..." 
            rows={2} 
          />

          <div className="grid gap-4 sm:grid-cols-2 pt-2 border-t border-brand-border dark:border-slate-800">
            <Select 
              label="Status" 
            ...
            <Select 
              ...
              onChange={(e) => setContentForm({ ...contentForm, visibility: e.target.value })}
              options={[
                { value: 'public', label: 'Public (Everyone)' },
                { value: 'private', label: 'Private (Enrolled)' }
              ]}
            />
          </div>

          {/* Conditional upload / editors */}
          {contentForm.type === 'notes' ? (
            <div className="space-y-1.5 pt-2 border-t border-brand-border dark:border-slate-800">
              <label className="text-xs font-semibold block text-brand-text-primary dark:text-slate-200">Notes Content</label>
              <RichNotesEditor 
                value={contentForm.markdown} 
                onChange={(val) => setContentForm({ ...contentForm, markdown: val })} 
              />
            </div>
          ) : contentForm.type === 'link' ? (
            <div className="space-y-3 pt-2 border-t border-brand-border dark:border-slate-800">
              <Input 
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Input 
                  label="Link Title" 
                  value={contentForm.linkTitle} 
                  onChange={(e) => setContentForm({ ...contentForm, linkTitle: e.target.value })} 
                  placeholder="Website name" 
                />
                <Input 
                  label="Link Description" 
                  value={contentForm.linkDescription} 
                  onChange={(e) => setContentForm({ ...contentForm, linkDescription: e.target.value })} 
            ...
            <div className="space-y-3 pt-2 border-t border-brand-border dark:border-slate-800">
              <label className="text-xs font-semibold block text-brand-text-primary dark:text-slate-200">Asset File</label>
              {/* File input */}
              <div 
                ...
                className="border-2 border-dashed border-brand-border dark:border-slate-800 rounded-xl p-6 text-center hover:border-accent-teal/40 transition-colors cursor-pointer bg-brand-surface dark:bg-slate-905"
                onClick={() => {
                  const input = document.createElement('input');
                  input.type = 'file';
                  // Mapped extensions based on contentForm.type
                  const mapExt = {
                    pdf: '.pdf',
                    ppt: '.ppt,.pptx',
                    pptx: '.ppt,.pptx',
              ...
              >
                <UploadCloud className="h-8 w-8 text-brand-text-secondary mx-auto mb-1 animate-pulse" />
                <p className="text-xs font-semibold">Drag & drop file here, or click to browse</p>
                <p className="text-[10px] text-brand-text-secondary mt-0.5">Max size 50MB</p>
              </div>
              ...
              {(contentSelectedFile || contentForm.fileUrl) && (
                <div className="p-3 rounded-lg border border-brand-border dark:border-slate-800 bg-brand-background flex items-center justify-between">
                  <div className="flex items-center gap-2 min-w-0">
                    <FileText className="h-4 w-4 text-accent-teal shrink-0" />
                    <span className="text-xs truncate font-semibold">{contentSelectedFile?.name || contentForm.fileUrl.split('/').pop()}</span>
                  </div>
                  <Button variant="outline" size="xs" onClick={handleModalCancelUpload} className="text-red-500 hover:text-red-650 shrink-0">
              ...
              {contentFileError && (
                <p className="text-[10px] font-semibold text-red-500 flex items-center gap-1"><AlertCircle className="h-3.5 w-3.5" /> {contentFileError}</p>
              )}
            </div>
          )}

          {/* Thumbnail */}
          <div className="pt-2 border-t border-brand-border dark:border-slate-800">
            <ImageUploader 
              label="Thumbnail Image (Optional)"
              value={contentForm.thumbnail} 
              onChange={(val) => setContentForm({ ...contentForm, thumbnail: val })}
              aspectRatio="video"
            />
          </div>
        </div>
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={cn(
              ...
              activeTab === tab 
                ? 'border-accent-teal dark:border-accent-teal text-accent-teal-dark dark:text-slate-100' 
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex rounded-lg border border-brand-border dark:border-slate-800 p-0.5 bg-white dark:bg-slate-900">
          <button type="button" onClick={() => setView('table')} className={cn('px-3 py-1 text-xs rounded-md font-medium', view === 'table' ? 'bg-accent-teal text-white' : 'text-brand-text-secondary dark:text-slate-400')}>Table</button>
          <button type="button" onClick={() => setView('card')} className={cn('px-3 py-1 text-xs rounded-md font-medium', view === 'card' ? 'bg-accent-teal text-white' : 'text-brand-text-secondary dark:text-slate-400')}>Cards</button>
        </div>
      </div>
